...
func _update_states_to_indicate() -> void:
	...
	if cleanliness_state and cleanliness_state_indicators.has(cleanliness_state.clean_state):
		current_states_to_indicate[cleanliness_state] = cleanliness_state_indicators[cleanliness_state.clean_state]
	
	if social_state and social_state_indicators.has(social_state.social_state):
		current_states_to_indicate[social_state] = social_state_indicators[social_state.social_state]
	
	if lifecycle_state and lifecycle_state_indicators.has(lifecycle_state.life_state):
		current_states_to_indicate[lifecycle_state] = lifecycle_state_indicators[lifecycle_state.life_state]
...
func _update_current_indicator() -> void:
	...
	print("Cleanliness state: ", cleanliness_state.clean_state, "Cleanliness value: ", cleanliness_state.cleanliness)
	print("Social state: ", social_state.social_state, "Social value: ", social_state.social_meter)
	print("Lifecycle state: ", lifecycle_state.life_state, "Lifecycle value: ", lifecycle_state.current_age)
